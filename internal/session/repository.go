...
func Create(dir, cwd string) (*Repository, error) {
	...
	id := newID()
	path := filepath.Join(dir, time.Now().UTC().Format("20060102_150405")+"_"+id+".jsonl")
	h := Header{Type: "session", Version: CurrentVersion, ID: id, Timestamp: time.Now().UTC().Format(time.RFC3339Nano), CWD: cwd}
